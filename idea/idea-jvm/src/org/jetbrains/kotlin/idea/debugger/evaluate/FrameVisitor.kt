...
class FrameVisitor(val context: EvaluationContextImpl) {
    ...
    private fun fail(message: String, shouldFail: Boolean): Value? {
        ...
        val sourceFileText = runReadAction { sourceFile?.text ?: "Source file text is not available" }

        val messageWithLocation = message + "\n\n" + locationText + "\n\n" + sourceFileText
        throw EvaluateExceptionUtil.createEvaluateException(messageWithLocation)
    }
